...
This program loads the individual FIFA datasets from
2015 to 2022, combines the loaded FIFA datasets, and
cleans the combined FIFA dataset from 2015 to 2022
for the machine learning tasks.
"""
...
# File path to save the cleaned, combined
# FIFA dataset from 2015 to 2022 ready
# to use for machine learning tasks.
SAVE_PATH = 'data/players_15_to_22_data.csv'


def load_data(path_15, path_16, path_17, path_18,
              path_19, path_20, path_21, path_22):
    """
    From the given file paths for individual FIFA
    datasets from 2015 to 2022, returns the combined
    FIFA data from 2015 to 2022 with irrelevant
    columns (usually repetitive or have a
    noticeable amount of missing values) as
    a pandas DataFrame.
    """
    ...
    players_15_to_22 = pd.concat([players_15, players_16,
                                  players_17, players_18,
                                  players_19, players_20,
...
def clean_player_positions(player_positions):
    """
    From the given player_positions (a String)
    with each position separated by commas,
    returns the cleaned player_positions
    with each position separated by spaces as
    a String.
    ...
    if type(player_positions) is float:
        return ""
    else:
...
def clean_work_rate(work_rates):
    """
    From the given work_rates (a String)
    with the attack work rate separated
    from the defense work rate by a forward
    slash, returns the cleaned work_rates
    with the attack work rate separated
    from the defense work rate by a space
    as a String.

    If there are no work_rates given (NaN),
    then an empty String is returned.
    """

    if type(work_rates) is float:
        return ""
    else:
...
def clean_player_traits(player_traits):
    """
    From the given player_traits (a String)
    with each trait separated by commes,
    returns the cleaned player_traits
    with each trait separated by spaces
    as a String.
    ...
    if type(player_traits) is float:
        return ""
    else:
...
def replace_missing_values(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas
    DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns the combined FIFA data from 2015
    to 2022 with the missing values (NaN) filled (determined
    by column type) and the removal of any players that are
    free agents as a pandas DataFrame.
    """
...
def player_positions_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas
    DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they play
    a certain position or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and
    the columns are the positions).
    """
    ...
    player_positions_data = pd.DataFrame(count_player_positions_matrix
                                         .toarray(),
                                         index=players_15_to_22_cleaned.index,
                                         columns=vectorizer_player_positions
                                         .get_feature_names_out())
    return player_positions_data


def work_rate_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas
    DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they have
    a certain workrate or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and
    the columns are the work rate types).
    """
    ...
    work_rate_data = pd.DataFrame(count_work_rate_matrix.toarray(),
                                  index=players_15_to_22_cleaned.index,
                                  columns=vectorizer_work_rate
                                  .get_feature_names_out())
    return work_rate_data


def player_traits_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas
    DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they have
    a certain trait or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and
    the columns are the traits).
    """
    ...
    player_traits_data = pd.DataFrame(count_player_traits_matrix.toarray(),
                                      index=players_15_to_22_cleaned.index,
                                      columns=vectorizer_player_traits
                                      .get_feature_names_out())
    return player_traits_data


def combine_data(players_15_to_22_cleaned, player_positions_data,
                 work_rate_data, player_traits_data):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame)
    cleaned combined FIFA data from 2015 to 2022, the given
    player_positions_data (a pandas DataFrame) data of
    whether a player plays a certain position or not, the given
    work_rate_data (a pandas DataFrame) data of whether a player
    has a certain work rate or not, and the given
    player_traits_data (a pandas DataFrame) data of whether a
    player has a certain trait or not, returns the cleaned,
    combined FIFA data from 2015 to 2022 ready to use for
    ...
    players_15_to_22_data = players_15_to_22_data.drop(columns=[
        'player_positions', 'work_rate', 'player_traits'])
    return players_15_to_22_data


def main():
    players_15_to_22_relevant = load_data(PATH_15, PATH_16,
                                          PATH_17, PATH_18,
                                          PATH_19, PATH_20,
                                          PATH_21, PATH_22)
    ...
    # columns.
    players_15_to_22_cleaned['player_positions'] = players_15_to_22_relevant[
        'player_positions'].apply(clean_player_positions)
    players_15_to_22_cleaned['work_rate'] = players_15_to_22_relevant[
        'work_rate'].apply(clean_work_rate)
    players_15_to_22_cleaned['player_traits'] = players_15_to_22_relevant[
        'player_traits'].apply(clean_player_traits)

    players_15_to_22_cleaned = replace_missing_values(
    ...
    players_15_to_22_data = combine_data(players_15_to_22_cleaned,
                                         player_positions_data,
                                         work_rate_data,
                                         player_traits_data)
