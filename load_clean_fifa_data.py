...
This program loads the individual FIFA datasets from 2015 to 2022, combines the loaded FIFA datasets, and
cleans the combined FIFA dataset from 2015 to 2022 for the machine learning tasks.
"""
...
def load_data(path_15, path_16, path_17, path_18, path_19, path_20, path_21, path_22):
    ...
    FIFA data from 2015 to 2022 with irrelevant columns (usually repetitive or have a noticeable amount
    of missing values) as a pandas DataFrame.
    """
...
def clean_work_rate(work_rates):
    """
    From the given work_rates (a String) with the attack work rate separated from the defense work rate by a forward
    slash, returns the cleaned work_rates with the attack work rate separated from the defense work rate by a space
    as a String.
...
def clean_player_traits(player_traits):
    """
    From the given player_traits (a String) with each trait separated by commes, returns the cleaned player_traits with
    each trait separated by spaces as a String.

    If there are no player_traits given (NaN), then an empty String is returned.
    """
...
def replace_missing_values(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns the combined FIFA data from 2015 to 2022 with the missing values (NaN) filled (determined
    by column type) and the removal of any players that are free agents as a pandas DataFrame.
    """
...
def player_positions_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they play a certain position or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and the columns are the positions).
    """
...
def work_rate_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they have a certain workrate or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and the columns are the work rate types).
    """
...
def player_traits_labels(players_15_to_22_cleaned):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame) cleaned combined FIFA data from 2015
    to 2022, returns for each player whether they have a certain trait or not (0 for no, 1 for yes) as a
    pandas DataFrame (each row represents a player and the columns are the traits).
    """
...
def combine_data(players_15_to_22_cleaned, player_positions_data, work_rate_data, player_traits_data):
    """
    From the given players_15_to_22_cleaned (a pandas DataFrame) cleaned combined FIFA data from 2015 to 2022, the given
    player_positions_data (a pandas DataFrame) data of whether a player plays a certain position or not, the given
    work_rate_data (a pandas DataFrame) data of whether a player has a certain work rate or not, and the given
    player_traits_data (a pandas DataFrame) data of whether a player has a certain trait or not, returns the cleaned,
    combined FIFA data from 2015 to 2022 ready to use for machine learning tasks as a pandas DataFramee.
    """
